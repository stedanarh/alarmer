...
begin
  ...
  SQLite3Connection1.DatabaseName := AppDir + 'alarmer.sqlite';
...
begin
  ...
  SQLQuery1.Close;
  SQLQuery1.SQL.Clear;
  SQLQuery1.SQL.Add('Select * From Usage Where StopTime Is Null');
  SQLQuery1.SQL.Add('And Computer=:Computer Order By Id Limit 1;');
  SQLQuery1.ParamByName('Computer').AsString := Computer;
  SQLQuery1.Open;
  ...
  SQLQuery1.Close;
  SQLQuery1.SQL.Clear;
  //SQLQuery1.SQL.Add('Update Usage Set StopTime=:Now Where StopTime Is Null;');
  //SQLQuery1.ParamByName('Now').AsDateTime := Now();
  SQLQuery1.SQL.Add('Delete From Usage;');
  SQLQuery1.ExecSQL;
  SQLTransaction1.Commit;

  SQLQuery1.Close;
  SQLQuery1.SQL.Clear;
  SQLQuery1.SQL.Add('Insert Into Usage(StartTime,Computer,Forced)');
  SQLQuery1.SQL.Add('Values (:Now,:Computer,0);');
  SQLQuery1.ParamByName('Now').AsDateTime := Now();
  SQLQuery1.ParamByName('Computer').AsString := Computer;
  SQLQuery1.ExecSQL;
  SQLTransaction1.Commit;
  ...
  SQLQuery1.Close;
  SQLQuery1.SQL.Clear;
  SQLQuery1.SQL.Add('Select * From Events /*Where Active = -1*/ Order By StopTime;');
  SQLQuery1.Open;
  while not SQLQuery1.EOF do
  begin
    Node1 := lvAlarms.Items.Add;
    Node1.Caption := IntToStr(lvAlarms.Items.Count);
    Node1.ImageIndex := SQLQuery1.FieldByName('Id').AsInteger;
    if SQLQuery1.FieldByName('Active').AsBoolean = True then
      Node1.SubItems.Add('Yes')
    else
      Node1.SubItems.Add('No');
    Node1.SubItems.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss',
      SQLQuery1.FieldByName('StartTime').AsDateTime));
    Node1.SubItems.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss',
      SQLQuery1.FieldByName('StopTime').AsDateTime));
    Length := DateUtils.SecondsBetween(SQLQuery1.FieldByName('StopTime').AsDateTime,
      SQLQuery1.FieldByName('StartTime').AsDateTime);
    Node1.SubItems.Add(Format('%d', [Length]));
    //Node1.SubItems.Add(Remaining);
    Node1.SubItems.Add(SecondsToTimeHMS(
      SecondsBetween(SQLQuery1.FieldByName('StopTime').AsDateTime, Now()), False));
    Node1.SubItems.Add(SQLQuery1.FieldByName('Message').AsString);
    SQLQuery1.Next;
  end;
  Timer1.Enabled := True;
end;
...
begin
  ...
  begin
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('Insert Into Events (Active,StartTime,StopTime,Message)');
    SQLQuery1.SQL.Add('Values (:Active,:StartTime,:StopTime,:Message)');
    SQLQuery1.ParamByName('Active').AsBoolean := True;
    SQLQuery1.ParamByName('StartTime').AsDateTime := StartTime;
    SQLQuery1.ParamByName('StopTime').AsDateTime := StopTime;
    SQLQuery1.ParamByName('Message').AsString := NewAlarmForm.meMessage.Text;
    SQLQuery1.ExecSQL;
    SQLTransaction1.Commit;
    SQLQuery1.Close;
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('Select Id From Events Where Active=:Active And');
    SQLQuery1.SQL.Add('StartTime=:StartTime And StopTime=:StopTime And Message=:Message');
    SQLQuery1.SQL.Add('Order By Id Desc Limit 1;');
    SQLQuery1.ParamByName('Active').AsBoolean := True;
    SQLQuery1.ParamByName('StartTime').AsDateTime := StartTime;
    SQLQuery1.ParamByName('StopTime').AsDateTime := StopTime;
    SQLQuery1.ParamByName('Message').AsString := NewAlarmForm.meMessage.Text;
    SQLQuery1.Open;
    IdAlarm := SQLQuery1.FieldByName('Id').AsInteger;
    SQLQuery1.Close;
...
begin
  ...
  if NewAlarmForm.Execute then
    ...
    begin
      ...
      SQLQuery1.ParamByName('StopTime').AsDateTime := StopTime;
      SQLQuery1.ParamByName('Message').AsString := NewAlarmForm.meMessage.Text;
      SQLQuery1.ParamByName('Id').AsInteger := MainForm.lvAlarms.Selected.ImageIndex;
      SQLQuery1.ExecSQL;
      SQLTransaction1.Commit;
      SQLQuery1.Close;
    end;
...
begin
  ...
  SQLQuery1.Close;
  SQLQuery1.SQL.Clear;
  SQLQuery1.SQL.Add('Update Usage Set StopTime=:Now,Forced=-1');
  SQLQuery1.SQL.Add('Where StopTime Is Null And Computer=:Computer;');
  SQLQuery1.ParamByName('Now').AsDateTime := Now();
  SQLQuery1.ParamByName('Computer').AsString := GetEnvironmentVariable('Computername');
  SQLQuery1.ExecSQL;
  SQLTransaction1.Commit;
  SQLQuery1.Close;
...
begin
  if lvAlarms.ItemIndex <> -1 then
    ...
    begin
      ...
      SQLQuery1.Close;
      SQLQuery1.SQL.Clear;
      SQLQuery1.SQL.Add('Update Events Set Active=:Active Where Id=:Id');
      SQLQuery1.ParamByName('Active').AsBoolean := True;
      SQLQuery1.ParamByName('Id').AsInteger := lvAlarms.Selected.ImageIndex;
      SQLQuery1.ExecSQL;
      SQLTransaction1.Commit;
      SQLQuery1.Close;
    end
    ...
    begin
      ...
      SQLQuery1.SQL.Clear;
      SQLQuery1.SQL.Add('Update Events Set Active=:Active Where Id=:Id');
      SQLQuery1.ParamByName('Active').AsBoolean := False;
      SQLQuery1.ParamByName('Id').AsInteger := lvAlarms.Selected.ImageIndex;
      SQLQuery1.ExecSQL;
      SQLTransaction1.Commit;
      SQLQuery1.Close;
...
begin
  ...
  begin
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('Delete From Events Where Active=0;');
    SQLQuery1.ExecSQL;
    SQLTransaction1.Commit;
    SQLQuery1.Close;
  end;
...
begin
  ...
  begin
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('Delete From Events Where Id=:Id');
    SQLQuery1.ParamByName('Id').AsInteger := lvAlarms.Selected.ImageIndex;
    SQLQuery1.ExecSQL;
    SQLTransaction1.Commit;
    SQLQuery1.Close;
...
begin
  ...
  begin
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('Select StartTime,StopTime From Events Where Id=:Id');
    SQLQuery1.ParamByName('Id').AsInteger := lvAlarms.Selected.ImageIndex;
    SQLQuery1.Open;
    Length := SecondsBetween(SQLQuery1.FieldByName('StartTime').AsDateTime,
      SQLQuery1.FieldByName('StopTime').AsDateTime);
    ...
    SQLQuery1.Close;
    SQLQuery1.SQL.Clear;
    SQLQuery1.SQL.Add('Update Events Set Active=:Active,');
    SQLQuery1.SQL.Add('StartTime=:StartTime,StopTime=:StopTime Where Id=:Id');
    SQLQuery1.ParamByName('Active').AsBoolean := True;
    SQLQuery1.ParamByName('StartTime').AsDateTime := StartTime;
    SQLQuery1.ParamByName('StopTime').AsDateTime := StopTime;
    SQLQuery1.ParamByName('Id').AsInteger := lvAlarms.Selected.ImageIndex;
  ...
  begin
    case ShowTheMessage('Alarmer', lvAlarms.Selected.SubItems[5],
      ...
      begin
        SQLQuery1.Close;
        SQLQuery1.SQL.Clear;
        SQLQuery1.SQL.Add('Update Events Set Active=0 Where Id=:Id;');
        SQLQuery1.ParamByName('Id').AsInteger := lvAlarms.Selected.ImageIndex;
        SQLQuery1.ExecSQL;
        SQLTransaction1.Commit;
        SQLQuery1.Close;
        ...
        if lvAlarms.Items[i - 1].SubItems[5] <> '' then
          case ShowTheMessage('Alarmer', lvAlarms.Items[i - 1].SubItems[5],
            ...
            begin
              ...
              SQLQuery1.SQL.Clear;
              SQLQuery1.SQL.Add('Update Events Set Active=0 Where Id=:Id;');
              SQLQuery1.ParamByName('Id').AsInteger := lvAlarms.Items[i - 1].ImageIndex;
              SQLQuery1.ExecSQL;
              SQLTransaction1.Commit;
              SQLQuery1.Close;
            end;
